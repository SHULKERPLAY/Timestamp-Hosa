...
**Timestamp 補佐** — это Discord-бот / пользовательское приложение, которое позволяет создавать любые метки времени для вставки в ваши сообщения, использовать калькулятор дат, вычислять время, прошедшее от одной даты до другой, рандомизировать дату, игровые кости или целые числа, а также конвертировать даты из формата UNIX в читаемый вид и наоборот! На данный момент поддерживает 11 языков: Русский, Українська, English, Français, Polski, Svenska, Deutsch, 日本語, Português (Brazilian), 한국어, Български.
...
-  `/random` - подфункции `integer`, `date` и `dice` - для генерации случайных значений
   - `integer` - создан чтобы сгенерировать любое случайное число из диапазона
      - `min`, `max` - определяет минимальное и максимальное возможное значение
   - `date` - создан чтобы сгенерировать любую случайную дату из диапазона
      - `fromyear`, `frommonth`, `fromday`, `fromhour`, `fromminute`, `fromsecond` - Минимально возможная дата
      - `toyear`, `tomonth`, `today`, `tohour`, `tominute`, `tosecond` - Максимально возможная дата
   - `dice` - создан чтобы симулировать бросок игровых костей (По умолчанию: D6)
      - `dicetype` - позволяет выбрать тип дайса из стандартных - D4, D6, D8, D10, D12, D20, D100
